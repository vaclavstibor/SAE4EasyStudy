...
def results():
    ...
    return render_template(
        ...
        journey_url_base=url_for(f"{PLUGIN_NAME}.participant_journey", participation_id=0).rstrip(
            "0"
        ),
...
def export_raw_data(guid):
    ...
    all_participations = (
        Participation.query.filter(Participation.user_study_id == user_study.id)
        .order_by(Participation.time_joined.asc())
        .all()
    )
    ...
    for participant in all_participations:
        ...
        if study_run:
                ...
                approach_runs.append(
                    {
                        ...
                        "started_at": approach.started_at.isoformat()
                        if approach.started_at
                        else None,
                        "completed_at": approach.completed_at.isoformat()
                        if approach.completed_at
                        else None,
            ...
            for event in (
                ...
                .order_by(SaeSteeringEvent.created_at.asc())
                .all()
            ):
            ...
            for feedback in (
                SaeMovieFeedback.query.filter(SaeMovieFeedback.study_run_id == study_run.id)
                .order_by(SaeMovieFeedback.created_at.asc())
                .all()
            ):
            ...
            for response in (
                SaeQuestionnaireResponse.query.filter(
                    SaeQuestionnaireResponse.study_run_id == study_run.id
                )
                .order_by(SaeQuestionnaireResponse.submitted_at.asc())
                .all()
            ):
            ...
            for search in (
                SaeFeatureSearch.query.filter(SaeFeatureSearch.study_run_id == study_run.id)
                .order_by(SaeFeatureSearch.created_at.asc())
                .all()
            ):
                hits = (
                    SaeFeatureSearchHit.query.filter(SaeFeatureSearchHit.search_id == search.id)
                    .order_by(SaeFeatureSearchHit.rank.asc())
                    .all()
                )
            ...
            for query in (
                SaeTextSteeringQuery.query.filter(SaeTextSteeringQuery.study_run_id == study_run.id)
                .order_by(SaeTextSteeringQuery.created_at.asc())
                .all()
            ):
                matches = SaeTextSteeringMatch.query.filter(
                    SaeTextSteeringMatch.query_id == query.id
                ).all()
            ...
            for example in (
                SaeExampleSteering.query.filter(SaeExampleSteering.study_run_id == study_run.id)
                .order_by(SaeExampleSteering.created_at.asc())
                .all()
            ):
                example_movies = (
                    SaeExampleSteeringMovie.query.filter(
                        SaeExampleSteeringMovie.example_id == example.id
                    )
                    .order_by(SaeExampleSteeringMovie.rank.asc())
                    .all()
                )
            ...
            for reset in (
                SaeResetAction.query.filter(SaeResetAction.study_run_id == study_run.id)
                .order_by(SaeResetAction.created_at.asc())
                .all()
            ):
        ...
        for pick in (
            SaeElicitationPick.query.filter(SaeElicitationPick.participation_id == participant.id)
            .order_by(SaeElicitationPick.created_at.asc())
            .all()
        ):
        ...
        participants_data.append(
            {
                ...
                "time_joined": participant.time_joined.isoformat()
                if participant.time_joined
                else None,
                "time_finished": participant.time_finished.isoformat()
                if participant.time_finished
                else None,
                ...
                "sae_study_run": {
                    ...
                    "started_at": study_run.started_at.isoformat()
                    if study_run.started_at
                    else None,
                    "finished_at": study_run.finished_at.isoformat()
                    if study_run.finished_at
                    else None,
    ...
    export = {
        ...
        "participants_completed": sum(
            1 for participant in participants_data if participant["time_finished"]
        ),
...
def _csv_response(study_run_ids: list[int], participation_ids: list[int]) -> dict:
    ...
    files["sae_approach_run.csv"] = write_rows(
        ...
        [
            [
                ...
                r.approach_name,
                r.steering_mode,
                r.enabled_modalities,
                r.sae_model_id,
                r.base_model_id,
                r.composition_mode,
                r.reranking_strategy,
                r.started_at.isoformat() if r.started_at else None,
                r.completed_at.isoformat() if r.completed_at else None,
                r.status,
                r.iterations_used,
                r.final_liked_count,
                r.total_slider_changes,
            ]
            for r in SaeApproachRun.query.filter(
                SaeApproachRun.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_steering_event.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeSteeringEvent.query.filter(
                SaeSteeringEvent.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_feature_adjustment.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeFeatureAdjustment.query.filter(
                SaeFeatureAdjustment.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_feature_search.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeFeatureSearch.query.filter(
                SaeFeatureSearch.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_feature_search_hit.csv"] = write_rows(
        ...
        [
            ...
            for r in (
                ...
                .filter(SaeFeatureSearch.study_run_id.in_(study_run_ids))
                .all()
                if study_run_ids
                else []
            )
    ...
    files["sae_text_steering_query.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeTextSteeringQuery.query.filter(
                SaeTextSteeringQuery.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_text_steering_match.csv"] = write_rows(
        ...
        [
            ...
            for r in (
                ...
                .filter(SaeTextSteeringQuery.study_run_id.in_(study_run_ids))
                .all()
                if study_run_ids
                else []
            )
    ...
    files["sae_example_steering.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeExampleSteering.query.filter(
                SaeExampleSteering.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_example_steering_movie.csv"] = write_rows(
        ...
        [
            ...
            for r in (
                ...
                .filter(SaeExampleSteering.study_run_id.in_(study_run_ids))
                .all()
                if study_run_ids
                else []
            )
    ...
    files["sae_reset_action.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeResetAction.query.filter(
                SaeResetAction.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_recommendation_set.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeRecommendationSet.query.filter(
                SaeRecommendationSet.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_recommendation_item.csv"] = write_rows(
        ...
        [
            ...
            for r in (
                ...
                .filter(SaeRecommendationSet.study_run_id.in_(study_run_ids))
                .all()
                if study_run_ids
                else []
            )
    ...
    files["sae_movie_feedback.csv"] = write_rows(
        ...
        [
            ...
            for r in SaeMovieFeedback.query.filter(
                SaeMovieFeedback.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
    ...
    files["sae_questionnaire_response.csv"] = write_rows(
        ...
        [
            [
                ...
                r.approach_index,
                r.approach_name,
                r.questionnaire_file,
                r.answers,
                r.submitted_at.isoformat() if r.submitted_at else None,
            ]
            for r in SaeQuestionnaireResponse.query.filter(
                SaeQuestionnaireResponse.study_run_id.in_(study_run_ids)
            ).all()
        ]
        if study_run_ids
        else [],
...
def export_csv_data(guid):
    ...
    response.headers["Content-Disposition"] = f"attachment; filename=study_{guid}_csv_export.zip"
    return response

...
The choice is captured on every `SaeApproachRun` row (`reranking_strategy` column) so retrospective analyses can correctly attribute outcomes to the active strategy. See [`equations.md` Section 10](equations.md#10-reranking-strategies-reranking_strategy-config-key) for the math and [`design-decisions.md` Section 23](design-decisions.md#23-reranking-strategies-reranking_strategy-config-key) for the rationale.
